...
import {
    ...
    EqpmntType,
    findEqpmntAC,
    initialEqpmntStateType,
    showAllEqpmntAC
} from "../../Redux/eqpmnt-reduser";
import {useDispatch, useSelector} from "react-redux";
import {AppRootStateType} from "../../Redux/redux-store";
import EquipmentCardWithRedux from "./EquipmentCard/EquipmentCardWithRedux";


export type CustomerCardPropsType = {
    // store: storeType
    //state: initialEqpmntStateType
}
...
function EquipmentList(props: CustomerCardPropsType) {
    ...
    // if (false) {
    //     equipment = equipmentState.filter(eq => eq.name === typeName)
    // }
    ...
    const findEqpmnt = () => {
        ...
        if (trimmedSn) {
            // dispatch(findEqpmntAC(trimmedTypeName, trimmedSn, trimmedDate, owner, "sn"))
            //setEquipment(equipmentState.filter(eq => eq.sn === sn))
            //setEquipmentState(props.store.getState().Equipment.filter(eq => eq.sn === sn))
        }
        if (trimmedDate) {
            // dispatch(findEqpmntAC(trimmedTypeName, trimmedSn, trimmedDate, owner, "date"))
            // setEquipment(equipmentState.filter(eq => eq.instDate === date))
            // setEquipmentState(props.store.getState().Equipment.filter(eq => eq.instDate === date))
        }
        if (trimmedOwner) {
            // dispatch(findEqpmntAC(trimmedTypeName, trimmedSn, trimmedDate, owner, "owner"))
            //setEquipment(equipmentState.filter(eq => eq.owner === owner))
    ...
    const eqpmnts = equipment.map((eq, index, array) => {

        return (
            <div key={eq.id}>
                <EquipmentCardWithRedux
                    id={eq.id}
                    ...
                    owner={eq.owner}
                    //store={props.store}
                    // setEquipmentState={() => setEquipment(equipmentState)}
                />
